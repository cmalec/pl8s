...
The [`UnobstructedArea`](https://developer.repebble.com/docs/c/User_Interface/UnobstructedArea/) API, added in SDK 4.0, allows developers to dynamically
...
overlays, but by using the [`UnobstructedArea`](https://developer.repebble.com/docs/c/User_Interface/UnobstructedArea/) API they can detect changes to
...
![Sample watchfaces with Timeline Quick View overlay](https://developer.repebble.com/assets/images/guides/user-interfaces/unobstructed-area/01-unobstructed-watchfaces.jpg)

Sample watchfaces with Timeline Quick View overlay

![Potential versions of sample watchfaces using the UnobstructedArea API](https://developer.repebble.com/assets/images/guides/user-interfaces/unobstructed-area/02-obstructed-watchfaces.jpg)

Potential versions of sample watchfaces using the
...
size of the display using [`layer_get_bounds()`](https://developer.repebble.com/docs/c/User_Interface/Layers/#layer_get_bounds) and then scale and position
your layers accordingly. Developers can now calculate the size of a layer,
excluding system obstructions, using the new [`layer_get_unobstructed_bounds()`](https://developer.repebble.com/docs/c/User_Interface/Layers/#layer_get_unobstructed_bounds).
...
If your application controls its own rendering process using a [`LayerUpdateProc`](https://developer.repebble.com/docs/c/User_Interface/Layers/#LayerUpdateProc) you can just dynamically adjust your rendering
each time your layer updates.

In this example, we use [`layer_get_unobstructed_bounds()`](https://developer.repebble.com/docs/c/User_Interface/Layers/#layer_get_unobstructed_bounds) instead of [`layer_get_bounds()`](https://developer.repebble.com/docs/c/User_Interface/Layers/#layer_get_bounds). The graphics are then positioned or scaled based upon
...
If you are not overriding the default rendering of a [`Layer`](https://developer.repebble.com/docs/c/User_Interface/Layers/#Layer), you will need to
subscribe to one or more of the [`UnobstructedAreaHandlers`](https://developer.repebble.com/docs/c/User_Interface/UnobstructedArea/#UnobstructedAreaHandlers) to adjust the sizes
and positions of layers.

There are 3 events available using [`UnobstructedAreaHandlers`](https://developer.repebble.com/docs/c/User_Interface/UnobstructedArea/#UnobstructedAreaHandlers).
These events will notify you when the unobstructed area is: *about to change*, *is currently changing*, or *has finished changing*. You can use these handlers
...
to change. This provides a [`GRect`](https://developer.repebble.com/docs/c/Graphics/Graphics_Types/#GRect) which lets you know the size of the screen
...
obstruction appearing or disappearing. [`AnimationProgress`](https://developer.repebble.com/docs/c/User_Interface/Animation/#AnimationProgress) is provided to let
...
available handlers.
> **NOTE**: You must construct the [`UnobstructedAreaHandlers`](https://developer.repebble.com/docs/c/User_Interface/UnobstructedArea/#UnobstructedAreaHandlers) object *before* passing it to the [`unobstructed_area_service_subscribe()`](https://developer.repebble.com/docs/c/User_Interface/UnobstructedArea/#unobstructed_area_service_subscribe) method.
...
In this example, we’re going to hide a [`TextLayer`](https://developer.repebble.com/docs/c/User_Interface/Layers/TextLayer/) containing the current
date, while the screen is obstructed.

Just before the Timeline Quick View appears, we’re going to hide the [`TextLayer`](https://developer.repebble.com/docs/c/User_Interface/Layers/TextLayer/) and we’ll show it again after the Timeline Quick View disappears.
...
![Unobstructed animation](https://developer.repebble.com/assets/images/guides/user-interfaces/unobstructed-area/unobstructed-animation.gif)
...
For design reference, the height of the Timeline Quick View overlay will be *51px* in total, which includes a 2px border, but this may vary on newer
