...
Source: https://developer.repebble.com/guides/tools-and-resources/app-metadata/
...
| --------------------------------- | ---------------- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`pebble.uuid`** | UUID | Developer-defined | Unique identifier [(UUID v4)](https://en.wikipedia.org/wiki/Universally_unique_identifier#Version_4_.28random.29) for the app. Generated by `pebble new-project`. |
...
| `pebble.targetPlatforms` | Array of strings | `["aplite", "basalt", "chalk"]` | Specify which platforms to build this app for. Read [*Hardware Information*](https://developer.repebble.com/guides/tools-and-resources/hardware-information/) for a list of available platforms. Defaults to all if omitted. |
| `pebble.enableMultiJS` | Boolean | `true` | Enables [use of multiple JS files](https://developer.repebble.com/blog/2016/01/29/Multiple-JavaScript-Files/) in a project with a CommonJS-style `require` syntax. |
...
| `pebble.capabilities` | Array of strings | None | List of capabilities that the app requires. The supported capabilities are `location`, `configurable` (detailed in [*PebbleKit JS*](https://developer.repebble.com/guides/communication/using-pebblekit-js/)), and `health` (detailed in [*Pebble Health*](https://developer.repebble.com/guides/events-and-services/health/)). |
| `pebble.messageKeys` | Object | `["dummy"]` | Keys used for [`AppMessage`](https://developer.repebble.com/docs/c/Foundation/AppMessage/) and [`AppSync`](https://developer.repebble.com/docs/c/Foundation/AppSync/). This is either a list of mapping of [`AppMessage`](https://developer.repebble.com/docs/c/Foundation/AppMessage/) keys. See [*PebbleKit JS*](https://developer.repebble.com/guides/communication/using-pebblekit-js/) for more information. |
| `pebble.resources.media` | Object | `[]` | Contains an array of all of the media resources to be bundled with the app (Maximum 256 per app). See [*App Resources*](https://developer.repebble.com/guides/app-resources) for more information. |
| `pebble.resources.publishedMedia` | Object | | Used for [*AppGlance Slices*](https://developer.repebble.com/guides/user-interfaces/appglance-c/) and [*Timeline Pins*](https://developer.repebble.com/guides/pebble-timeline/pin-structure/). See [Published Media](#published-media) for more information. |

> Note: `hiddenApp` and `onlyShownOnCommunication` are mutually exclusive. `hiddenApp` will always take preference.
...
specify image resources which can be used within [*AppGlance Slices*](https://developer.repebble.com/guides/user-interfaces/appglance-c/) or [*Timeline Pins*](https://developer.repebble.com/guides/pebble-timeline/pin-structure/).
...
We'll start by declaring some image resources which we can reference in `publishedMedia` later. In this example we have 'hot' and 'cold' icons, in 3
different sizes.
...
For example, the AppGlance C API prefixes the name: `PUBLISHED_ID_WEATHER_HOT` but the other APIs use: `app://images/WEATHER_HOT`.
...
[`AppGlanceSlice`](https://developer.repebble.com/docs/c/Foundation/App_Glance/#AppGlanceSlice). Size: 25x25.
