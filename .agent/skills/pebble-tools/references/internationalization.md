...
The Pebble SDK allows an app to be localized with the
[`Internationalization`](https://developer.repebble.com/docs/c/Foundation/Internationalization/) API.
Developers can use this to adjust how their app displays and behaves to cater
for users in non-English speaking countries. By default all apps are assumed to
be in English. Choose the user selected locale using
[`i18n_get_system_locale()`](https://developer.repebble.com/docs/c/Foundation/Internationalization/#i18n_get_system_locale)
or to force one using [`setlocale()`](https://developer.repebble.com/docs/c/Standard_C/Locale/#setlocale).

> Note: The app's locale also affects
> [`strftime()`](https://developer.repebble.com/docs/c/Standard_C/Time/#strftime), which will output different
> strings for different languages. Bear in mind that these strings may be
> different lengths and use different character sets, so the app's layout should
> scale accordingly.
...
| Language   | Region        | Value returned by [`i18n_get_system_locale()`](https://developer.repebble.com/docs/c/Foundation/Internationalization/#i18n_get_system_locale) |
| ---------- | ------------- | ---------------------------------------------------------------------------------------------------------------- |
...
internationalization. Use the value returned by
[`setlocale()`](https://developer.repebble.com/docs/c/Standard_C/Locale/#setlocale) to translate any app
strings. The implementation follows the
...
standard [`navigator`](http://www.w3schools.com/jsref/prop_nav_language.asp)
property:
...
It is also possible to explicitly set an app's locale using
[`setlocale()`](https://developer.repebble.com/docs/c/Standard_C/Locale/#setlocale). This is useful for
providing a language selection option for users who wish to do so. This function
takes two arguments; the `category` and the `locale` string. To set localization
**only** for the current time, use `LC_TIME` as the `category`, else use
`LC_ALL`. The `locale` argument accepts the ISO locale code, such as `"fr_FR"`.
Alternatively, set `locale` to `NULL` to query the current locale or `""` (an
empty string) to set the app's locale to the system default.
...
This will change the locale used to display time and dates in
[`strftime()`](https://developer.repebble.com/docs/c/Standard_C/Time/#strftime) without translating any
strings.

## Effect of Setting a Locale

Besides returning the value of the current system locale, calling
[`setlocale()`](https://developer.repebble.com/docs/c/Standard_C/Locale/#setlocale) will have a few other
effects:

- Translate month and day names returned by
  [`strftime()`](https://developer.repebble.com/docs/c/Standard_C/Time/#strftime) `%a`, `%A`, `%b`, and `%B`.

- Translate the date and time representation returned by
  [`strftime()`](https://developer.repebble.com/docs/c/Standard_C/Time/#strftime) `%c`.

- Translate the date representation returned by
  [`strftime()`](https://developer.repebble.com/docs/c/Standard_C/Time/#strftime) `%x`.
...
framework works by wrapping every string in the project's `src` directory in the
`_()` and replacing it with with a hashed value, which is then used with the
current locale to look up the translated string from a binary resource file for
that language.
...
- Add the output binary files as project resources as described in
  [*Raw Data Files*](https://developer.repebble.com/guides/app-resources/raw-data-files/).

When the app is compiled and run, the `_()` macro will be replaced with calls to
`locale_str()`, which will use the app's locale to load the translated string
