...
- **Draw Command Frame** — a Draw Command List with a configurable duration, making up one animation frame. Many of these are used in a Draw Command Sequence.
- **Draw Command Image** — a single vector image.
- **Draw Command Sequence** — a set of Draw Command Frames that make up an animated sequence of vector images.
...
All offsets below are relative to the start of the component, except for the Image and Sequence tables, whose offsets are relative to the start of the enclosing **file** (hence the jump to `8`, past the 8-byte file header).
...
Both sizes are 4 bytes and the payload starts at offset 8, which is why the component tables above begin at offset `8`.
