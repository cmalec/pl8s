# <img src="appstore/icon-48.png" width="38" alt="pl8s icon"> pl8s ("plates")
...
- **Settings wizard** (hold SELECT): first set the percent step (5% or 10%), then step through each plate size (55/45/35/25/15/10/5/2.5 lb) and set how many the gym has: 0-10 per side, or 99 for unlimited (the default)
...
GitHub Pages works out of the box for this repo. The page talks to the watch via the standard Pebble config protocol, and it mirrors **everything the on-watch settings wizard can change** — percent step, exercise max, and per-plate counts — prefilled with the current values:

1. The watch pushes its current settings when asked: `src/pkjs/index.js` sends a
   `SYNC` AppMessage on JS ready and on every gear tap; `src/c/main.c` replies with `STEP`, `MAX_LB`, and all `PLATE_*` values. The JS side caches them in localStorage and appends them to the config URL as query params.
2. The phone app's gear fires `showConfiguration`, which opens
   `CONFIG_URL + '?step=..&max=..&p55=..&p2p5=..'`; `config.html` prefills the form from those params.
...
The watch-only wizard (hold SEL) and the phone page can be used interchangeably; the watch pushes its state after the wizard closes too, so the page always opens showing current values.
...
Testing phone-side pushes from the emulator. `--int` takes the numeric message keys the SDK assigns, listed in `build/js/message_keys.json` after a build, and all pairs go after one `--int`:
...
Built for **emery** (Pebble Time 2) first, with layout adaptations for the smaller rect displays (basalt/diorite/flint/aplite) and round displays (chalk/gabbro). On the round displays the rows step inward with the circular mask: each row's text and its plate glyph are inset to the widest chord that row has, so nothing is cut off by the glass.
...
The plate math is plain C with no Pebble SDK dependency, so it is unit tested on the host against the same source the watchapp links:

```sh
npm test
```

`test/test_plate_math.c` compares every row against an independent oracle (a memoized exhaustive search over plate multisets, deliberately a different algorithm from the app's reachable-load bitset) across the whole max range, both step modes, curated and random limited inventories, and every inventory with 0..2 of each size. It also pins the plates the glyph draws, and the rows that a limited inventory has to snap down.

The app itself only really runs on the watch, so the wizard, persistence and rendering are checked on the emulator:
...
That restarts the emery emulator, drives the settings wizard with button presses and reads the weights back off the screen with `tools/ocr_weights.py`, so a wrong weight fails the run instead of just changing the picture.
